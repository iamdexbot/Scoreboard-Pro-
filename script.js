...
function closeTimeoutSetup() {
  document.getElementById('timeoutSetupModal').classList.remove('open');
}
function setMaxTimeouts(n) {
  n = Math.max(1, Math.min(10, n));
  // Update the constant-like variable
  window.MAX_TIMEOUTS = n;
  // Reset both teams to the new max
  homeTimeouts = n;
  awayTimeouts = n;
...
function applyCustomTimeouts() {
  ...
  if (!isNaN(val) && val >= 1 && val <= 10) {
    ...
    document.getElementById('customToInput').focus();
  }
}
document.addEventListener('DOMContentLoaded', function() {
  var toModal = document.getElementById('timeoutSetupModal');
  if (toModal) toModal.addEventListener('click', function(e) {
    if (e.target === this) closeTimeoutSetup();
  });
});
